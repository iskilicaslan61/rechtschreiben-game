...
export default function AnlauteGame({ questions, onScore, onComplete }: Props) {
  ...
  return (
    <div>
        ...
        {questions.map((q, i) => (
          <div
            ...
            className={`bg-white rounded-2xl border-2 p-4 flex items-center gap-3 transition-all duration-200
              ...
              ${statuses[i] === 'idle' ? 'border-blue-100 hover:border-blue-300 hover:shadow-sm' : ''}`}
          >
            <span className="text-4xl min-w-[52px] text-center drop-shadow">{q.emoji}</span>
            <input
              type="text"
              maxLength={1}
              value={values[i] ?? ''}
              ...
              placeholder="?"
              className="w-14 text-center text-2xl font-black border-b-4 border-dashed border-blue-300
                bg-transparent text-blue-900 outline-none uppercase focus:border-blue-600 transition-colors
                placeholder:text-blue-200"
            />
            <span className="text-xl min-w-[28px] text-center">
      ...
      <div className="flex justify-center mt-5">
        <button
          onClick={check}
          className="px-8 py-3 bg-gradient-to-b from-blue-500 to-blue-700 text-white font-bold rounded-full
            shadow-[0_4px_14px_rgba(59,130,246,0.4)] hover:shadow-[0_6px_20px_rgba(59,130,246,0.5)]
